...
class OtpBloc extends Bloc<OtpEvent, OtpState> {
  ...
  void _onStartTimer(StartTimer event, Emitter<OtpState> emit) {
    _timer?.cancel();
    emit(state.copyWith(secondsRemaining: _duration, canResend: false));
  ...
  void _onOtpInputChanged(OtpInputChanged event, Emitter<OtpState> emit) {
    emit(state.copyWith(enteredOtp: int.tryParse(event.code), errorText: null));
  }

  Future<void> _onResendOtpRequested(ResendOtpRequested event, Emitter<OtpState> emit) async {
    if (state.isResending) return;
    emit(state.copyWith(isResending: true, errorText: null));
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        emit(state.copyWith(receivedOtp: finalOtp, isResending: false));
        add(StartTimer()); // Restart countdown
      } else {
        emit(state.copyWith(isResending: false, errorText: result['message'] ?? "Resend failed"));
      }
    } catch (e) {
      emit(state.copyWith(isResending: false, errorText: "Connection error"));
    }
  }
  ...
  Future<void> _onVerifyOtpRequested(VerifyOtpRequested event, Emitter<OtpState> emit) async {
    ...
    try {
      ...
      emit(state.copyWith(isVerifying: false, isSuccess: true, profile: profile));
    } catch (e) {
      emit(state.copyWith(isVerifying: false, errorText: e.toString()));
    }
  }
