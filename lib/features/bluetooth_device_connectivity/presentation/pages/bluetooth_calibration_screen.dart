...
class BluetoothCalibrationScreen extends StatefulWidget {
  ...
  const BluetoothCalibrationScreen({super.key, required this.calibrationParams,});
...
class _BluetoothCalibrationScreenState
  ...
  void _navigateToDashboard(BluetoothCalibrationCubit cubit) {
    ...
    context.go(AppRoutes.clientDashboard, extra: widget.calibrationParams.clientProfileModel);
  }
      ...
      showDeviceDisconnectedBox(
        ...
        onButtonPressed: () async {
          ...
          context.go(
            ...
            extra: widget.calibrationParams.clientProfileModel,
          );
        },
      ).then((_) => _dialogOpen = false);
    });
        ...
        listener: (context, state) {
          ...
          if (state.navigateToInhaleScreen) {
            _postFrame(() {
              ...
              final params = InhaleScreenParams(
                  clientProfileModel: widget.calibrationParams.clientProfileModel,
                  dietPlanStrategyModel: widget.calibrationParams.dietPlanStrategyModel,
                  minRange: widget.calibrationParams.minRange,
                  maxRange: widget.calibrationParams.maxRange,
                  breathingSettings:state.breathingSettings,
                  featuresAllowData: widget.calibrationParams.featuresAllowData,
                  userHabitsModel: widget.calibrationParams.userHabitsModel
              );
              context.go(AppRoutes.bluetoothInhaleScreen, extra: params,);
            });
            return;
          }
